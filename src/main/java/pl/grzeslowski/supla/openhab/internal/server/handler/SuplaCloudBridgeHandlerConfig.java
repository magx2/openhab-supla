...
public class SuplaCloudBridgeHandlerConfig {
    ...
    public boolean isValid() {
        var serverAuth = serverAccessId != null
                && //
                serverAccessIdPassword != null
                && !serverAccessIdPassword.isBlank();
        var emailAuth = email != null && !email.isBlank() && authKey != null && !authKey.isBlank();
        return (serverAuth || emailAuth) && port.intValue() > 0;
    }

    @Override
    public String toString() {
        return "SuplaCloudBridgeHandlerConfig{" + //
                "serverAccessId="
                + serverAccessId + //
                ", serverAccessIdPassword='<SECRET>'"
                + //
                ", email='"
                + email + '\'' + //
                ", authKey='<SECRET>'"
